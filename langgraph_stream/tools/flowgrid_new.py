...
class FlowGrid(object):
    def __init__(self):
        ...
        self.Prop = {}
        self.out_dir = 'output'
        #self.times= []  # added for reading well data.
...
class CMG(FlowGrid):
    ...
    def __init__(self):
        ...
        self.out_dir = 'output'
        self.out_props = {}
        self.times= []  # trying this peice here instead of main class.
    ...
    def _str_to_float(self, s):
        if not s[0].isdigit():
            s = s.replace('–', '-').replace(' ', '')  # Replace en-dash with negative sign
        else:
            if self._is_float(s):
                s = float(s)
            else:
    ...
    def bound_type(self, fname):
    # Initialize the bound array with default value -1
        bound = np.full((1, 1, 1, self.size[2], self.size[1], self.size[0]), -1)

    # Search for the word AQUIFER or *AQUIFER in the .dat file
        with open(fname, 'r') as file:
             for line in file:
                if 'AQUIFER' in line or '*AQUIFER' in line:
                # Populate bound array with 1 if found
                    bound.fill(1)
                    break
        return bound 
        
 # This below is the original code. The code was modified to parse negative floats in the output file        
